...
local M = Class 'UILogic'

---@class UILogic: Storage
Extends('UILogic', 'Storage')
---@class UILogic: GCHost
Extends('UILogic', 'GCHost')





---@class UILogic.OnRefreshInfo
---@field name string
---@field on_refresh fun(ui?: UI, player: Player, instance: UILogic)

---@class UILogic.OnInitInfo
---@field name string
---@field on_init fun(ui: UI, player: Player, instance: UILogic)

---@class UILogic.OnEventInfo
---@field name string
---@field event y3.Const.UIEvent
---@field on_event fun(ui: UI, player: Player, instance: UILogic)

---@class UILogic.PrefabInfo
---@field child_name string
---@field prefab_logic UILogic
---@field prefab_token any

---@param path_or_ui? string | UI
---@param player Player
function M:__init(path_or_ui, player)
    ---@private
    self._bind_unit_attr = {}
    ---@package
    ---@type UILogic.OnRefreshInfo[]
    self._on_refreshs = {}
    ---@package
    ---@type UILogic.OnEventInfo[]
    self._on_events = {}
    ---@package
    ---@type UILogic.OnInitInfo[]
    self._on_inits = {}
    ---@package
    ---@type UILogic.PrefabInfo[]
    self._prefab_infos = {}
    ---@package
    ...
    y3.timer.wait(0, function()
        if type(path_or_ui) == 'string' then
            local main = y3.ui.get_ui(player, path_or_ui)
            self:attach(main)
        end
...
function M:attach(ui, kv)
    ...
    ---@private
    ---@type table<string, UILogic.OnRefreshInfo[]>
    ...
    ---@package
    ---@type table<string, table<string, UILogic[]>>
...
---@param on_refresh fun(ui?: UI, Player: Player, instance: UILogic)
...
---@param callback fun(ui: UI, Player: Player, instance: UILogic)
...
---@param child_name string # 空字符串表示主控件
---@param on_init fun(ui: UI, Player: Player, instance: UILogic)
...
--绑定元件
---@param child_name string # 空字符串表示主控件
---@param prefab_logic UILogic # 使用 `y3.local_ui.prefab` 创建的元件逻辑
...
---@param on_refresh? fun(ui?: UI, Player: Player, instance: UILogic) # 刷新元件前的回调，你可以趁机通过 `instance:storage_set` 设置元件的属性。
function M:refresh_prefab(prefab_token, count, on_create, on_refresh)
    ...
    for _, info in ipairs(self._prefab_infos) do
            ...
            if count < #instances then
                ...
                for i = #instances + 1, count do
                    ...
                    if instance then
                        ...
                        local ui = y3.ui_prefab.create(self._player, info.prefab_logic._prefab_name, parent):get_child()
        ...
        for _, instance in ipairs(instances) do
            if on_refresh then
                xpcall(on_refresh, log.error, instance._main, self._player, instance)
            end
            instance:refresh('*')
        end
        ::continue::
...
function M:register_events()
    for _, info in ipairs(self._on_events) do
        ...
        if ui then
            ui:add_fast_event(info.event, function()
                info.on_event(ui, self._player, self)
            end)
        else
...
function M:init()
    for _, info in ipairs(self._on_inits) do
        local ui = self._childs[info.name]
        if ui then
            xpcall(info.on_init, log.error, ui, self._player, self)
        else
            log.error('控件不存在！', info.name)
        end
    end
...
function M:refresh(name)
    ...
    if not self._refresh_next_tick_timer then
        self._refresh_next_tick_timer = y3.timer.wait(0, function()
    ...
    for name in pairs(need_refresh) do
        ...
        for _, info in ipairs(infos) do
            ...
            if ui then
                if not mark[ui] then
                    mark[ui] = true
                    xpcall(info.on_refresh, log.error, ui, self._player, self)
                end
            else
                log.error('控件不存在！', info.name)
            end
...
---@class UILogic.API
---@overload fun(player: Player):self
local API = Class 'UILogic.API'

---@param player Player
---@return self
function API:__init(player)
    self._player = player
    return self
end
...
function API.new(player)
    local api = New 'UILogic.API' (player)
    return api
end
...
function API:create(path_or_ui)
    local local_ui = New 'UILogic' (path_or_ui, self._player)
    return local_ui
end
...
function API:prefab(prefab_name)
    local logic = New 'UILogic' (nil, self._player)
